...
MINIMAL_NAV_COLUMNS = ['timestamp', 'state', 'altitude', 'velocity x', 'velocity y', 'velocity z',
                       'acceleration x', 'acceleration y', 'acceleration z']
...
if __name__ == '__main__':
    if len(sys.argv) != 3 or '-h' in sys.argv or '--help' in sys.argv:
        ...
        sys.exit(1)
    navdata = pandas.read_csv(sys.argv[1], sep='\t', names=NAVDATA_COLUMNS, header=None, usecols=MINIMAL_NAV_COLUMNS)
    ...
    for WINDOW_LENGTH, NAVDATA_OFFSET_MILIS in [(10,10), (15,10), (15,15), (20,10), (20,15)]:
        ...
        np.save("joined_data_long_minimal_%d_%d" % (WINDOW_LENGTH, NAVDATA_OFFSET_MILIS), joined_data)
        # np.save("joined_data_minimal_%d_%d" % (WINDOW_LENGTH, NAVDATA_OFFSET_MILIS), joined_data)
        np.save("new_navdata_long_minimal_%d_%d" % (WINDOW_LENGTH, NAVDATA_OFFSET_MILIS), new_navdata)
        # np.save("new_navdata_minimal_%d_%d" % (WINDOW_LENGTH, NAVDATA_OFFSET_MILIS), new_navdata)
